...
class IAMClient {
    ...
    createAccount(accountName, options, callback) {
        ...
        this.request('IAMPOST', '/', (err, result) => {
            if (err) {
                return callback(err);
            }
    ...
    listAccounts(options, callback) {
        ...
        this.request('IAMPOST', '/', callback, data);
    }
    ...
    checkPolicies(requestContextParams, userArn, options, callback) {
        ...
        this.request('S3POST', '/', (err, data, code) => {
            if (err) {
                return callback(err);
            }
            return callback(null, {
    ...
    request(method, path, callback, data, reqUid) {
        ...
        if (method === 'IAMPOST' || method === 'S3POST') {
            httpMethod = 'POST';
        }
        const options = {
            method: httpMethod,
